...
def test_html_measurement_handles_void_raw_text_unicode_and_recovery() -> None:
    ...
    assert [element["tag"] for element in result["elements"]] == [
        "html",
        "body",
        "br",
        "script",
    ...
    assert result["errorOrMissingNodes"] == 1
...
def test_lf_and_crlf_produce_the_same_physical_ranges() -> None:
    lf = scan_bytes(b"<root>\n<item/>\n</root>\n", "xml")
    crlf = scan_bytes(b"<root>\r\n<item/>\r\n</root>\r\n", "xml")

    assert lf["physicalLines"] == crlf["physicalLines"] == 3
    assert lf["elements"] == crlf["elements"]
